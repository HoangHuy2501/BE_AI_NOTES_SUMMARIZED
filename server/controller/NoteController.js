...
async function GetNotes(req,res, next) {
    try {
        const user_id = req.params.id;
        const result = await noteServices.getNotes(user_id, req.query);
        // console.log("result", result);
        
        return res.json(ApiSuccess.getSelect("Note",result));
    } catch (error) {
        return next(error);
    }
}
// lấy 5 notes gần nhất
async function GetRecentNotes(req,res, next) {
    try {
        const user_id = req.params.id;
        const result = await noteServices.getRecentNotes(user_id);
        // console.log("result", result);
        
        return res.json(ApiSuccess.getSelect("Recent Notes", result.rows));
    } catch (error) {
        return next(error);
    }
}
async function GetNoteById(req,res, next) {
    try {
        ...
        return res.json(ApiSuccess.getSelect("Note by ID", result.rows));
    } catch (error) {
        return next(error);
    }
}
...
module.exports = {CreatNote,GetNotes,GetNoteById,PutNote,DeleteNote, GetRecentNotes};